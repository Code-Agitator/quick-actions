...
- **模块 ID**: plugin-independent-packaging
- **复杂度等级**: L2 - 中等
- **预估代码量**: 400-600 行（包括模板、脚本更新、测试）
- **相关文件**: 
  - `scripts/templates/pack-plugin-standalone.js` (新建)
  - `plugins/*/scripts/pack-plugin.js` (新建，6个文件)
  - `plugins/*/package.json` (修改，6个文件)
  - `scripts/create-plugin.js` (修改)
  - `scripts/pack-plugin.js` (标记废弃)
...
- [ ] **独立脚本可执行**: `node plugins/uuid-generator/scripts/pack-plugin.js` 能正常运行
- [ ] **无需参数**: 脚本自动检测父目录作为插件根目录
...
- [ ] **开始提示**: 显示 "🚀 Packing plugin..."
- [ ] **进度信息**: 显示 "📂 Reading plugin metadata..."
- [ ] **元数据确认**: 显示 "✅ Plugin ID: {id}" 和 "✅ Version: {version}"
...
- [ ] **复制到临时目录**: 插件可复制到任意位置并正常工作
- [ ] **重新安装依赖**: 在临时目录运行 `pnpm install` 成功
- [ ] **独立打包**: 在临时目录运行 `pnpm pack` 成功
- [ ] **无外部引用**: 不依赖 `../../scripts/pack-plugin.js`

**测试步骤**:
```bash
# 1. 复制插件到临时目录
cp -r plugins/uuid-generator /tmp/test-independence
cd /tmp/test-independence
...
- [ ] **archiver 已安装**: 每个插件有自己的 archiver 实例
- [ ] **版本一致**: 所有插件使用相同版本的 archiver (^8.0.0)
- [ ] **无共享依赖**: 不依赖根目录的 node_modules/archiver

**验证命令**:
```bash
# 检查每个插件的 archiver 安装情况
for plugin in uuid-generator everything-search js-console json-explorer process-manager qa-test-plugin; do
  echo "Checking $plugin..."
  ls plugins/$plugin/node_modules/archiver/package.json
done
# 每个插件都应显示 archiver 的 package.json
```
...
- [ ] **旧脚本仍可用**: `node ../../scripts/pack-plugin.js <plugin-name>` 仍可工作
- [ ] **废弃警告**: 旧脚本显示清晰的废弃警告
- [ ] **ZIP 格式一致**: 新旧脚本生成的 ZIP 内容完全一致
- [ ] **命令接口不变**: `pnpm pack` 命令仍然有效
...
```javascript
// ✅ 正确
async function packPlugin(pluginDir) { ... }
 ...
 * @param {string} pluginDir - The name of the plugin directory
...
### 5.1 代码风格（user-conventions.md 第 2.2 节）
...
```bash
# 手动检查行数
wc -l plugins/uuid-generator/scripts/pack-plugin.js
# 应 < 300
...
### 5.2 错误处理（user-conventions.md 第 2.5 节）
...
### 5.3 文档要求（user-conventions.md 第 4 节）
 ...
 */
async function createZipArchive(zipFilePath, distDir, pluginJsonPath, pluginDir, zipFileName) {
...
**测试方法**:
```bash
# 使用 Measure-Command (PowerShell) 或 time (Unix)
Measure-Command { pnpm pack }
# TotalSeconds 应 < 2
```
...
**测试方法**（PowerShell）:
```powershell
Measure-Command { node scripts/pack-plugin.js }
```
...
# 计算 ZIP 大小
(Get-Item uuid-generator-1.0.0.zip).Length / 1MB
...
### 8.1 create-plugin.js 更新

- [ ] **package.json 模板**: 新生成的插件包含 `"pack": "node ./scripts/pack-plugin.js"`
- [ ] **scripts 目录**: 新插件自动创建 `scripts/pack-plugin.js`
- [ ] **.gitignore 模板**: 包含 `*.zip` 规则
- [ ] **README 模板**: 包含打包使用说明
- [ ] **archiver 依赖**: 新插件的 devDependencies 包含 `"archiver": "^8.0.0"`
...
cat package.json | grep archiver
# 应输出: "archiver": "^8.0.0"
```

### 8.2 现有插件更新

所有 6 个插件的 `package.json` 必须更新：

- [ ] `plugins/uuid-generator/package.json` - 添加 pack 脚本和 archiver 依赖
- [ ] `plugins/everything-search/package.json` - 添加 pack 脚本和 archiver 依赖
- [ ] `plugins/js-console/package.json` - 添加 pack 脚本和 archiver 依赖
- [ ] `plugins/json-explorer/package.json` - 添加 pack 脚本和 archiver 依赖
- [ ] `plugins/process-manager/package.json` - 添加 pack 脚本和 archiver 依赖
- [ ] `plugins/qa-test-plugin/package.json` - 添加 pack 脚本和 archiver 依赖
...
foreach ($plugin in $plugins) {
    Write-Host "Checking $plugin..."
    $packageJson = Get-Content "plugins\$plugin\package.json" | ConvertFrom-Json
    
    # 检查 pack 脚本
    if ($packageJson.scripts.pack -eq "node ./scripts/pack-plugin.js") {
        Write-Host "  ✅ pack script correct"
    } else {
        Write-Host "  ❌ pack script incorrect: $($packageJson.scripts.pack)"
    }
    
    # 检查 archiver 依赖
    if ($packageJson.devDependencies.archiver) {
        Write-Host "  ✅ archiver dependency present"
    } else {
        Write-Host "  ❌ archiver dependency missing"
    }
}
```
...
**验证命令**:
```powershell
Get-Content scripts/create-plugin.js | Select-String "\.zip"
# 应看到 *.zip 在生成的 .gitignore 内容中
```

### 8.4 文档更新

- [ ] **DEVELOPMENT_GUIDE.md**: 更新插件打包说明，替换手动 zip 命令为 `pnpm pack`
- [ ] **README.md 模板**: 新插件 README 包含打包说明
- [ ] **迁移指南**: 创建 `docs/MIGRATION_TO_INDEPENDENT_PACKAGING.md`

**验证内容**（DEVELOPMENT_GUIDE.md）:
```markdown
### Pack Plugin
...
创建测试脚本 `scripts/test-independent-packaging.js`（可选但推荐）:

```javascript
#!/usr/bin/env node
/**
 * Automated test for independent plugin packaging
 */
...
const pluginDir = path.join(__dirname, '..', 'plugins', testPlugin);

console.log('🧪 Running independent packaging tests...\n');
...
// Test 2: Pack plugin with new script
console.log('Test 2: Packing plugin with independent script...');
try {
  execSync('node ./scripts/pack-plugin.js', { 
    cwd: pluginDir, 
    stdio: 'inherit' 
  });
...
console.log('Test 4: Testing independence...');
const tempDir = path.join(__dirname, '..', 'temp-test-independence');
try {
  // Copy plugin to temp directory
  execSync(`xcopy /E /I "${pluginDir}" "${tempDir}"`, { stdio: 'pipe' });
  ...
  // Build and pack
  execSync('pnpm build', { cwd: tempDir, stdio: 'pipe' });
  execSync('pnpm pack', { cwd: tempDir, stdio: 'pipe' });
  
  // Verify
  const tempZip = path.join(tempDir, `${testPlugin}-1.0.0.zip`);
  if (fs.existsSync(tempZip)) {
    console.log('✅ Independence test passed\n');
  } else {
    console.error('❌ Independence test failed');
    process.exit(1);
  }
...
#### 步骤 1: 新建插件测试
- [ ] 运行 `node scripts/create-plugin.js test-independent-pack`
- [ ] 进入目录 `cd plugins/test-independent-pack`
- [ ] 运行 `pnpm install`
- [ ] 运行 `pnpm build`
- [ ] 运行 `pnpm pack`
- [ ] 验证生成了 `test-independent-pack-1.0.0.zip`
- [ ] 清理测试插件 `cd ../.. && Remove-Item -Recurse -Force plugins\test-independent-pack`

#### 步骤 2: 现有插件测试（对所有 6 个插件）
...
**文档版本**: v1.0  
**最后更新**: 2026-05-09  
**关联规划**: `.lingma/plans/plugin-independent-packaging.md`
